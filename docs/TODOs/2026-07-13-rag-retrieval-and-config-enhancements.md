...
- [x] 在 dependencies 加 `"bm25s>=0.2.0"`；`uv lock` + `uv sync`
- [x] 验证 `uv run python -c "import bm25s; print(bm25s.__version__)"` 成功
- [x] commit `chore(chatbot): 加 bm25s 依赖`
...
- [x] 写测试：build 小语料 → query 关键词返回正确 chroma_id 排序；save→load 往返 query 结果一致
- [x] 实现 + `uv run pytest tests/test_bm25_index.py -v` 通过
- [x] commit `feat(chatbot): Bm25Index（bm25s 包装 + 持久化）`
...
- [x] 测试：build_and_save 后 get 命中；invalidate 后 get 重新加载
- [x] 实现 + 测试通过；commit `feat(chatbot): Bm25Registry 进程级缓存`
...
- [x] 删除原 `kb_path` 字段，加 `kb_dirs` + property；注意 `expand_path` validator 改为对 list 每项展开。
- [x] `uv run pytest -q` 现有用例不回归（kb_path property 兼容）
- [x] commit `refactor(chatbot): Settings kb_path→kb_dirs + retrieval 配置`
...
- [x] 测试：两个目录各放文件 → 返回成对 (root,file)；同名文件不丢
- [x] 实现；commit `feat(chatbot): iter_documents 支持多目录`
...
- [x] `uv run pytest tests/test_config_store.py -q` 通过；加 kb_dirs 往返测试
- [x] commit `feat(chatbot): RAG 配置支持多目录与检索算法`
...
- [x] 测试：两目录同名文件 → 两个 chunk 都保留（count=2），metadata.path 含 root.name 前缀
- [x] 实现 + `uv run pytest tests/test_vector_store.py -q`；commit `feat(chatbot): build 支持多目录 + chunk id 防冲突`
...
- [x] 测试：构造索引，`hybrid` 命中关键词优于纯 vector；`bm25` only 与 `vector` only 各自可用；bm25 缺失 + method=hybrid → is_rag_ready False
- [x] 实现 + 测试；commit `feat(chatbot): search 支持 hybrid RRF 融合`
...
- [x] 扩展 `tests/test_supervisor.py`：submit 完成后 `~/.porto/bm25s_index/` 存在，`Bm25Registry.get` 非空
- [x] `uv run pytest tests/test_supervisor.py -q`；commit `feat(chatbot): reindex 同步构建 BM25 索引`

## Task 10: 前端 types.ts RagConfig 新字段
**Files:** Modify `chatbot/frontend/src/lib/types.ts`
- [x] `RagConfig` 加 `kb_dirs: string[]`、`retrieval_method: "vector"|"bm25"|"hybrid"`、`bm25_top_k: number`；`defaultRagConfig`（api.ts）同步默认值
- [x] `npx tsc --noEmit` 通过；commit `feat(chatbot): RagConfig 新增 kb_dirs/retrieval 字段`
...
- [x] tsc 通过；commit `feat(chatbot): sources 显示目录相对路径`
...
- [x] tsc 通过；commit `refactor(chatbot): critic 提升为独立 agent 卡片`
...
- [x] tsc 通过；commit `feat(chatbot): RAG 保存与 Re-index 分离`
...
- [x] tsc 通过；commit `feat(chatbot): 多目录管理 + 检索算法选择 UI`

## Task 15: 端到端验证
- [x] `cd chatbot/backend && uv run pytest -q` 全绿
- [x] `cd chatbot/frontend && npx tsc --noEmit` 无错误
- [x] commit `test(chatbot): 端到端验证（如无新改动则空 commit 跳过）`
