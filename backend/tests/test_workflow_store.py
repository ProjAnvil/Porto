...
def test_clear_outputs_after(tmp_path):
    ...
    s.clear_outputs_after(wid, "understand")  # 删 identify/generate(Task 10:evaluate 已删)
    outs = s.get_outputs(wid)
...
def test_list_filters(tmp_path):
    ...
    s.update_status(w1, "completed", current_step="generate")  # Task 10:evaluate 已删
...
def test_update_spec_updates_only_named_spec(tmp_path):
    ...
    # Task 10:evaluate 节点已删,不再造 evaluate output;用 retrieve 作"下游不变"对照
    s.save_output(wid, "retrieve", {"sources": []}, "ai")
    ...
    # 其他 step 的产出不应被 PATCH /specs 改动
    assert outs["retrieve"]["output"] == {"sources": []}
